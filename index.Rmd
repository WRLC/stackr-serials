...
ws_key_exists <- file.exists("./wcapi.json")
...
wc_get_token <- function() {
  ...
  write(token, file = "./wcapi_token.json")
  ...
  if(nrow(new) > 0) {
    ...
    tmp <- sapply(seq_along(new$oclc_num),
                  function(i) {
                    ...
                    if(file.exists('./wcapi_token.json')) {  # If so...
                      tokenfile <- fromJSON('./wcapi_token.json')  # ...get the stored token
                      ...
                      if(token_exp <= st) {  # If it's expired...
                        ...
                        if(!is.null(ref_token) & !is.null(ref_exp)) {  # if there is a refresh token...
                          if(ref_exp <= st) { # ...but the refresh token is expired, too...
                            ...
                            token <- fromJSON('./wcapi.json')  # ...and assign it to `token`
                          } else {  # but if refresh token isn't expired...
                            refresh_wc_token(token)  # ...use the refresh token to get a new token
                            token <- fromJSON('./wcapi_token.json')  # ...and assign it to `token`
                          }
